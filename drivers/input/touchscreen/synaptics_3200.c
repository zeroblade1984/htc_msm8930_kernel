...
unsigned int double_tap_barrier_y = 1300;
...
static void dt2w_func(int x, int y, cputime64_t trigger_time) {
	if (x < 50 || x > (1080-50) || y < 50 || y > (1920-50) || (dt2w_switch && (y > 0 
						&& y < double_tap_barrier_y))) {
		reset_dt2w();
		return;
	}
...
static int report_htc_logo_area(int x, int y) {
	...
	if (last_touch_position_x > 380 && last_touch_position_x < 700) {
		// M4 x=380 x=700
		if (last_touch_position_y > 1850 || (scr_suspended == true && last_touch_position_y > 1780)) {
			// M4 y=1850 y=1780 
